...
>[!NOTE]
> Дополнительные сведения о планировании ресурсов и производительности см. в статье [Планирование производительности Azure ATP](capacity-planning.md).

В состав Azure ATP входит облачная служба Azure ATP, которая включает портал Azure ATP и датчик Azure ATP. Дополнительные сведения о каждом компоненте Azure ATP см. в статье [Архитектура Azure ATP](architecture.md).
...
- При попытке установить датчик Azure ATP на компьютере, настроенном с функцией объединения сетевых карт, возникает ошибка установки. Чтобы установить датчик Azure ATP на компьютере с функцией объединения сетевых карт, ознакомьтесь с [соответствующей проблемой](troubleshooting-known-issues.md#nic-teaming).
...
- Необязательно: При развертывании автономного датчика следует пересылать [события Windows](configure-windows-event-collection.md#configure-event-collection) в Azure ATP, чтобы дополнительно улучшить обнаружение угроз, связанных с проверкой подлинности, добавлением к конфиденциальным группам и подозрительным созданием служб.  Датчик Azure ATP получает эти события автоматически. В автономном датчике Azure ATP эти события можно получать из системы SIEM или настроив пересылку событий Windows с контроллера домена. Собранные события предоставляют Azure ATP дополнительные данные, которые невозможно получить через сетевой трафик контроллера домена.
...
Разрешение сетевых имен (NNR) — главный компонент функций Azure ATP. Чтобы разрешить IP-адреса для имен компьютеров, датчики Azure ATP запрашивают IP-адрес, используя один из следующих методов:
...
Для работы первых трех методов необходимо открыть соответствующие порты для входящего трафика с датчиков Azure ATP на устройства в сети. Дополнительные сведения о политике NNR в Azure ATP см. в [этой статье](nnr-policy.md).
...
Дополнительные сведения о требованиях к оборудованию для датчика Azure ATP см. в статье [Планирование производительности Azure ATP](capacity-planning.md).
...
|------------|-------------|--------|-----------|
|**Интернет-порты**|||||
|SSL (*.atp.azure.com)|TCP|443|Датчик Azure ATP|Облачная служба Azure ATP|
|SSL (localhost)|TCP|444|Датчик Azure ATP|localhost|
|**Внутренние порты**|||||
|DNS|TCP и UDP|53|Датчик Azure ATP|DNS-серверы|
|Netlogon (SMB, CIFS, SAM-R)|TCP/UDP|445|Датчик Azure ATP|Все устройства в сети|
|RADIUS|UDP|1813|RADIUS|Датчик Azure ATP|
|**Порты NNR**\*|||||
|HTML через RPC|TCP|Порт 135|Датчики Расширенной защиты от угроз|Все устройства в сети|
|NetBIOS|UDP|137|Датчики Расширенной защиты от угроз|Все устройства в сети|
|RDP|TCP|3389 (только первый пакет приветствия клиента)|Датчики Расширенной защиты от угроз|Все устройства в сети|
...
Дополнительные сведения о требованиях к оборудованию для автономного датчика Azure ATP см. в статье [Планирование производительности Azure ATP](capacity-planning.md).
...
|SSL (localhost)|TCP|444|Датчик Azure ATP|localhost|
|**Внутренние порты**||||
|LDAP|TCP и UDP|389|Датчик Azure ATP|Контроллеры домена|
|Защищенный LDAP (LDAPS)|TCP|636|Датчик Azure ATP|Контроллеры домена|
|LDAP для глобального каталога|TCP|3268|Датчик Azure ATP|Контроллеры домена|
|LDAPS для глобального каталога|TCP|3269|Датчик Azure ATP|Контроллеры домена|
|Kerberos|TCP и UDP|88|Датчик Azure ATP|Контроллеры домена|
|Netlogon (SMB, CIFS, SAM-R)|TCP и UDP|445|Датчик Azure ATP|Все устройства в сети|
|Служба времени Windows|UDP|123|Датчик Azure ATP|Контроллеры домена|
|DNS|TCP и UDP|53|Датчик Azure ATP|DNS-серверы|
|Syslog (необязательно)|TCP/UDP|514, в зависимости от конфигурации|Сервер SIEM|Датчик Azure ATP|
|RADIUS|UDP|1813|RADIUS|Датчик Azure ATP|
|**Порты NNR** \*|||||
|HTML через RPC|TCP|135|Датчики Расширенной защиты от угроз|Все устройства в сети|
|NetBIOS|UDP|137|Датчики Расширенной защиты от угроз|Все устройства в сети|
|RDP|TCP|3389 (только первый пакет приветствия клиента)|Датчики Расширенной защиты от угроз|Все устройства в сети|
