...
1.  Использование мастер-ключа. Проверить заражение контроллеров домена вредоносными программами, использующими мастер-ключи, можно с помощью [сканера, созданного группой разработчиков Azure ATP](https://gallery.technet.microsoft.com/Aorato-Skeleton-Key-24e46b73).
...
3. В случае отрицательных ответов на вопросы выше вы, вероятнее всего, столкнулись с вредоносными программами.
...
## <a name="suspicious-service-creation---preview-feature"></a>Создание подозрительной службы — функция предварительного просмотра!

**Описание**

На контроллере домена в вашей организации была создана подозрительная служба. Это оповещение связано с событием 7045 для определения этого подозрительного действия на конечных точках. Событие 7045 следует перенаправить из конечных точек в ATP путем настройки [пересылки событий Windows](configure-event-forwarding.md) или путем пересылки событий 7045 в SIEM и [настройки SIEM](configure-event-collection.md) в качестве источника данных, который перенаправляет события в ATP.
...
>![ПРИМЕЧАНИЕ] Чтобы отключить подозрительное действие, обратитесь в службу поддержки.
