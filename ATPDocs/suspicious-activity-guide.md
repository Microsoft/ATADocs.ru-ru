...
-   **Положительные.** Служба Azure ATP обнаружила вредоносное действие.

-   **Неопасные положительные.** Служба Azure ATP обнаружила действие, которое реальное, но невредоносное, например тест на проникновение.

-   **Ложные положительные.** Ложное оповещение о действии, которое не произошло.
...
2. Если да и так должно быть, **закройте и исключите** предупреждение.

3. Если да, но это действие должно отсутствовать, **закройте** предупреждение.
...
## <a name="suspected-golden-ticket-usage-nonexistant-account"></a>Предполагаемое использование Golden Ticket (несуществующая учетная запись)
...
Злоумышленники с правами администратора домена могут скомпрометировать учетную запись KRBTGT. Используя учетную запись KRBTGT, они могут создать билет на получение билетов Kerberos (TGT), с помощью которого можно получить доступ к любому ресурсу. Подложный билет TGT этого типа называют Golden Ticket (золотой билет), так как он позволяет злоумышленникам добиться длительной сохраняемости сети. Подложные билеты Golden Ticket этого типа имеют уникальные характеристики, для выявления которых предназначено это обнаружение. 

**Исследование**
1. Службы федерации могут создавать билеты, которые будут активировать это оповещение. Размещаются ли такие службы на исходном компьютере? Если да, закройте оповещение системы безопасности.
2. Щелкните имя исходного пользователя, чтобы перейти на страницу профиля. Проверьте, что происходило во время этих действий.
      1. Предполагается ли доступ (разрешен ли доступ) пользователя к этому ресурсу? 
      2.  Предполагается ли, что участник имеет доступ к этой службе? 
3. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля. Просмотрите, что происходило во время этих действий, обращая особое внимание на необычные действия, в том числе на то, кто выполнял вход и к каким ресурсам обращался этот пользователь. 
4. Все ли пользователи, которые вошли в систему компьютера, должны были это делать? Какие у них права доступа? 
5. Должны ли пользователи, которые входили в компьютер, иметь доступ к ресурсам, на которых они выполнили вход?
<br>Если вы включили интеграцию ATP в Защитнике Windows, щелкните эмблему ATP в Защитнике Windows.
6. Для дальнейшей проверки компьютера в ATP Защитника Windows просмотрите, какие процессы выполнялись и какие оповещения возникали в интересующий вас период.


**Исправление**

Несколько раз меняйте пароль билета на получение билетов Kerberos (KRBTGT), как описано в записи блога о [скриптах для сброса пароля к учетной записи KRBTGT, доступных клиентам](https://blogs.microsoft.com/microsoftsecure/2015/02/11/krbtgt-account-password-reset-scripts-now-available-for-customers/) с помощью [средства сброса пароля или ключей к учетной записи KRBTGT](https://gallery.technet.microsoft.com/Reset-the-krbtgt-account-581a9e51). Учтите, что такой сброс приведет к тому, что все билеты Kerberos в этом домене станут недействительными: тщательно спланируйте сброс. Так как при создании Golden Ticket требуются права администратора домена, следуйте [рекомендациям касательно атак типа Pass-the-Hash](https://www.microsoft.com/download/details.aspx?id=36036).
...
*Предыдущее название.* Подозрительное изменение привилегированных групп

**Описание**

Обычно злоумышленники добавляют пользователей в группы с высоким уровнем привилегий. Это позволяет им получить постоянный доступ к большему числу ресурсов и добиться постоянного присутствия в системе. Обнаружение основано на профилировании действий по изменению группы пользователей и отправке оповещений при наличии аномального изменения привилегированной группы. Служба Azure ATP выполняет профилирование непрерывно. Минимальный срок запуска оповещения составляет один месяц в каждом контроллере домена.
...
Изучите необычное действие в оповещении системы безопасности на временной шкале действий. Щелкните оповещение системы безопасности, чтобы перейти на страницу подробностей, а затем просмотрите потенциально затронутые сущности и список свидетельств. 

Это *положительное*, *неопасное положительное* или *ложное положительное* оповещение? 

1. Проверьте, запущена ли на исходном компьютере программа WannaCry. 

2. Если запущена, это предупреждение является положительным. Чтобы определить область бреши в системе безопасности, сделайте следующее:
      - Исследуйте исходный компьютер.
      - Исследуйте скомпрометированный компьютер. 

2. Если на исходном компьютере не выполняется средство взлома, причиной этого могут быть приложения, которые реализуют собственный стек SMB или NTLM. Проверьте, выполняются ли на исходном компьютере такие приложения.

      1. Если на компьютере не должен реализовываться собственный стек, но это происходит, исправьте конфигурацию приложения. В таком случае это неопасное действие, и оповещение системы безопасности можно закрыть.

      2. Если на компьютере реализуется собственный стек и его конфигурация задана правильно, можно закрыть оповещение системы безопасности и исключить компьютер, так как это, вероятнее всего, неопасное действие.

3. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля. Просмотрите, что происходило в период вызова оповещения, обращая особое внимание на необычные действия. Отследите, кто выполнял вход и к каким ресурсам обращался этот пользователь. 
...
2. Если запущено, это предупреждение является положительным. Чтобы определить область бреши в системе безопасности, сделайте следующее:
      - Исследуйте исходный компьютер.
      - Исследуйте скомпрометированный компьютер. 

3. Если на исходном компьютере не выполняется средство взлома, причиной этого могут быть приложения, которые реализуют собственный стек SMB или NTLM. Проверьте, выполняются ли на исходном компьютере такие приложения.

4. Если на компьютере не должен реализовываться собственный стек NTML или SMB, но это происходит, исправьте конфигурацию приложения.
      1. В таком случае это неопасное действие, и оповещение системы безопасности можно закрыть. 
      2. Если на компьютере реализуется собственный стек и его конфигурация задана правильно, можно закрыть оповещение системы безопасности и исключить компьютер, так как это, вероятнее всего, неопасное действие.

5. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля. Просмотрите, что происходило в тот период, когда было вызвано оповещение, обращая особое внимание на необычные действия. Отследите, кто выполнял вход и к каким ресурсам обращался этот пользователь. Если вы включили интеграцию ATP в Защитнике Windows, щелкните эмблему ATP в Защитнике Windows ![эмблема wd](./media/wd-badge.png) , чтобы получить дополнительные диагностические данные. ATP в Защитнике Windows предоставляет информацию о том, какие процессы выполнялись и какие оповещения возникали в интересующий вас период.


**Исправление**

1. Сбросьте пароли скомпрометированных пользователей и включите многофакторную аутентификацию.
...
4. Сбросьте пароли пользователя исходного компьютера и включите многофакторную аутентификацию. 
5. [Удалите SMBv1](https://blogs.technet.microsoft.com/filecab/2016/09/16/stop-using-smb1/).
...
Злоумышленники используют средства, которые реализуют различные протоколы (например, SMB, Kerberos) нестандартными способами. Хотя этот тип сетевого трафика принимается Windows без каких-либо предупреждений, Azure ATP может распознавать потенциально вредоносные цели. Например, когда поведение указывает на такие методы, как Over-Pass-the-Hash и атаку методом подбора, а также эксплойты, используемые дополнительными программами-шантажистами, например WannaCry.
...
Это *положительное*, *неопасное положительное* или *ложное положительное* оповещение? 
 ...
 1. Иногда приложения реализуют собственный стек Kerberos без соответствия Kerberos RFC.
   1. Проверьте, используется ли на исходном компьютере собственный стек Kerberos. 
   2. Если на компьютере не должен реализовываться собственный стек Kerberos, но это происходит, исправьте конфигурацию приложения. В таком случае это неопасное действие, и оповещение системы безопасности можно закрыть. 
   3. Если на компьютере реализуется собственный стек Kerberos и его конфигурация задана правильно, можно закрыть оповещение системы безопасности и исключить компьютер, так как это, вероятнее всего, неопасное действие.
...
1. Проверьте, запущено ли на исходном компьютере такое средство взлома, как Hydra. 
   1. Если запущено, это предупреждение является положительным. Чтобы определить область бреши в системе безопасности, сделайте следующее:
      - Исследуйте исходный компьютер.
      - Исследуйте скомпрометированный компьютер. 

2. Если на исходном компьютере не выполняется средство взлома, причиной этого могут быть приложения, которые реализуют собственный стек SMB или NTLM. Проверьте, выполняются ли на исходном компьютере такие приложения.

3. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля. Просмотрите, что происходило в период вызова оповещения, обращая особое внимание на необычные действия. Отследите, кто выполнял вход и к каким ресурсам обращался этот пользователь. Если вы включили интеграцию ATP в Защитнике Windows, щелкните эмблему ATP в Защитнике Windows ![эмблема wd](./media/wd-badge.png) , чтобы получить дополнительные диагностические данные. ATP в Защитнике Windows предоставляет информацию о том, какие процессы выполнялись и какие оповещения возникали в интересующий вас период.

**Исправление**

1. Сбросьте пароли предполагаемых пользователей и включите многофакторную проверку подлинности.
2. Добавьте предполагаемых пользователей в список отслеживания.
3. Уменьшите влияние исходного компьютера.
   1. Найдите средство, инициировавшее атаку, и удалите его.
   2. Найдите пользователей, выполнивших вход во время вредоносной активности, они также могут быть скомпрометированы.
   3. Сбросьте их пароли и включите многофакторную проверку подлинности. 
4. Настройте принудительное применение длинных и сложных паролей в организации. Длинные и сложные пароли обеспечивают необходимый первый уровень защиты от атак методом подбора.
5. [Удалите SMBv1](https://blogs.technet.microsoft.com/filecab/2016/09/16/stop-using-smb1/).
...
**Описание** При атаке методом подбора злоумышленник пытается пройти проверку подлинности с помощью подбора разных паролей к учетным записям, пока не получится подобрать правильный пароль по крайней мере к одной из них. Когда злоумышленник находит пароль, он может войти в систему с помощью соответствующей учетной записи.

При таком обнаружении оповещение активируется вследствие неоднократных неудачных попыток аутентификации с помощью Kerberos или NTLM. Это может быть обнаружение атак методом подбора по горизонтали (небольшой набор паролей для большого числа пользователей) или по вертикали (большой набор паролей для небольшого числа пользователей). Кроме того, может использоваться любое сочетание этих вариантов.

**Временной интервал** Минимальный срок активации оповещения составляет одну неделю после развертывания датчика.

**Исследование**

1. Щелкните оповещение для просмотра: 
   - Список учетных записей, подвергшихся атаке.
   - Список учетных записей, для которых попытки предположительно завершились успешной аутентификацией.
...
2. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля. Проверьте, что происходило во время попыток, обращая особое внимание на необычные действия. Отследите, кто выполнял вход и к каким ресурсам он обращался. Если вы включили интеграцию ATP в Защитнике Windows, щелкните значок ATP в Защитнике Windows для дальнейшего исследования компьютера. Чтобы просмотреть информацию о том, какие процессы выполнялись и какие оповещения возникали в интересующий вас период, воспользуйтесь ATP в Защитнике Windows.

3. Если аутентификация выполнялась с помощью NTLM и это предупреждение возникало много раз, но у вас недостаточно сведений о том, к какому серверу исходный компьютер пытался получить доступ, включите аудит NTLM на соответствующих контроллерах домена. Включите аудит NTLM на соответствующих контроллерах домена, включив событие 8004. Это событие аутентификации NTLM содержит сведения об исходном компьютере, учетной записи пользователя и сервере, к которому выполнялся доступ. Выяснив, какой сервер выполнял проверку подлинности, исследуйте сервер, проверив события, в частности событие 4624, чтобы отследить весь процесс аутентификации.
<br>
1. Щелкните **Download details** (Загрузить сведения), чтобы просмотреть всю информацию в формате электронной таблицы Excel. 

**Исправление**

Длинные сложные пароли обеспечивают необходимый первый уровень защиты от атак методом подбора.
...
*Предыдущее название.* Рекогносцировка с использованием запросов к службам каталогов <a name="reconnaissance-using-directory-service-queries"></a>
...
**Период обучения** 4 недели для контроллера домена, начиная с первой сетевой активности SAMR относительно определенного контроллера домена. 

Это **истинно-положительное**, **неопасное истинно-положительное** или **ложное положительное** оповещение? 

1. Щелкните имя исходного компьютера, чтобы перейти на страницу профиля.        — Должен ли исходный компьютер генерировать действия этого типа?  
      - Если да, то вы можете *закрыть* оповещение системы безопасности и исключить этот компьютер, так как это неопасное истинно-положительное действие. 
2. Проверьте пользователя/пользователей, выполнивших операцию. 
      - Выполнили ли они вход на исходный компьютер обычным образом или они являются администраторами, которые могут выполнять такие действия?   
      - Проверьте профиль пользователя и его соответствующие действия. Поймите их обычное поведение и выполните поиск других подозрительных действий с помощью руководства для исследования. 
      - Если да, *закройте* оповещение системы безопасности как неопасное действие. 
...
5. Настройте доступ к сети: ограничьте число клиентов, которые могут выполнять удаленные вызовы групповой политики SAM.
...
2. Просмотрите, какой именно пользователь выполнил действие. Проверьте, выполнили ли они вход на исходный компьютер стандартным образом или они являются администраторами, которые могут выполнять такие действия.  

3. Если да и оповещение обновилось, **подавите** вывод оповещения системы безопасности.  

4. Если да, но это действие должно отсутствовать, **закройте** оповещение системы безопасности.
