...
## <a name="choosing-the-right-sensor-type-for-your-deployment"></a>Выбор правильного типа датчика для развертывания
...
Датчик Azure ATP поддерживает отслеживание одного контроллера домена в зависимости от объема сетевого трафика, создаваемого контроллером домена. В следующей таблице приводятся приблизительные данные, так как окончательный объем, анализируемый датчиком, зависит от имеющегося объема трафика. 
...
> [!NOTE]
> - Общее количество ядер без применения технологии Hyper-Threading, установленных на этом контроллере домена.<br>Хотя технология Hyper-Threading является приемлемой для датчика Azure ATP, при планировании производительности необходимо подсчитать фактические ядра и ядра без применения технологии Hyper-Threading.
> - Общий объем памяти, установленной в этом контроллере домена.
> -   Если на контроллере домена нет ресурсов, необходимых для датчика Azure ATP, это не повлияет на производительность контроллера домена, но датчик Azure ATP, возможно, не будет работать должным образом.
...
> -   Требуется не менее двух ядер и шести ГБ пространства. Мы рекомендуем 10 ГБ. Сюда входит пространство, необходимое для двоичных файлов Azure ATP.
