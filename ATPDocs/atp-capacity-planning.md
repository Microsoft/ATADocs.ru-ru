...
- Скачайте [средство изменения размера Azure ATP](http://aka.ms/aatpsizingtool).
...
- Общее количество ядер, которые будет использовать служба датчика.<br>Рекомендуем не использовать ядра с поддержкой технологии Hyper-Threading.
...
- При запуске в качестве виртуальной машины функция перераспределения динамической памяти или любой другой памяти не поддерживается.
