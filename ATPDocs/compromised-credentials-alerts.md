...
> - [Подозрение на атаку AS-REP Roasting (внешний идентификатор: 2412)](#suspected-as-rep-roasting-attack-external-id-2412)
...
Дополнительные сведения об учетных записях Honeytoken см. в разделе [Настройка учетных записей Honeytoken и исключений из обнаружения](configure-detection-exclusions.md).
...
При обнаружении такой проблемы активируется оповещение, если [!INCLUDE [Product short](includes/product-short.md)] фиксирует значительное количество проверок подлинности с простой привязкой. Это оповещение определяет атаки методом подбора *по горизонтали* (небольшой набор паролей для большого числа пользователей) либо *по вертикали* (большой набор паролей для небольшого числа пользователей) или любое сочетание этих вариантов. Оповещение основано на событиях проверки подлинности, обнаруженных датчиками, которые установлены на контроллере домена и AD FS серверах.
...
1. Сбросьте пароль исходного пользователя и включите MFA или, если вы настроили соответствующие политики пользователей с высоким риском в Защите идентификации Azure Active Directory, вы можете использовать действие [**Подтвердить компрометацию пользователя**](/cloud-app-security/accounts#governance-actions) на портале Cloud App Security.
1. Настройте в организации требование [длинных и сложных паролей](/windows/device-security/security-policy-settings/password-policy), которые обеспечивают необходимый первый уровень защиты от атак методом подбора.
1. Откажитесь от использования протокола передачи открытого текста LDAP в своей организации.
...
Либо можно применить к этому оповещению код **TP**. Следуйте инструкциям в разделе **Определение области бреши в системе безопасности**.
...
## <a name="suspected-as-rep-roasting-attack-external-id-2412"></a>Подозрение на атаку AS-REP Roasting (внешний идентификатор: 2412)

Злоумышленники с помощью специальных средств обнаруживают учетные записи, в которых отключена *предварительная проверка подлинности Kerberos*, и отправляют запросы AS-REQ без зашифрованной метки времени. В ответ они получают сообщения AS-REP с данными TGT, которые зашифрованы с помощью ненадежного алгоритма, такого как RC4, и сохраняют эти данные для использования при взломе пароля без подключения к Интернету (как при атаке Kerberoasting) и предоставления учетных данных в незашифрованном виде.
...
1. Уменьшите влияние исходного компьютера.
    - Найдите средство, инициировавшее атаку, и удалите его.
    - Найдите пользователей, выполнивших вход в систему во время вредоносной активности, так как эти пользователи также могут быть скомпрометированы. Сбросьте их пароли и включите MFA, или, если вы настроили соответствующие политики пользователей с высоким риском в Защите идентификации Azure Active Directory, вы можете использовать действие [**Подтвердить компрометацию пользователя**](/cloud-app-security/accounts#governance-actions) на портале Cloud App Security.
1. Включите предварительную проверку подлинности Kerberos. Дополнительные сведения об атрибутах учетной записи и способах их исправления см. в статье [Оценка безопасности. Небезопасные атрибуты учетной записи](cas-isp-unsecure-account-attributes.md).
