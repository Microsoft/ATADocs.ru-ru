...
description: В этой статье рассматриваются оповещения Azure ATP, которые возникают, когда обнаруживаются атаки на организацию, связанные с этапом компрометации учетных данных.
keywords: ''
author: shsagir
ms.author: shsagir
manager: shsagir
...
3. [Боковое смещение](lateral-movement-alerts.md)
4. [Полное управление доменом](domain-dominance-alerts.md)
5. [Кража данных](exfiltration-alerts.md)
...
> * Предполагаемая попытка повышения прав доступа к службе Netlogon (внешний идентификатор: 2411)
...
1. Проверьте, выполняются ли на компьютере-источнике приложения, реализующие собственный стек SMB или NTLM.
    ...
    2. Если на компьютере-источнике выполняется такое приложение, и это закономерное поведение, **закройте** оповещение системы безопасности как действие **B-TP** и исключите компьютер.
...
## <a name="suspected-netlogon-privilege-elevation-attempt-cve-2020-1472-exploitationexternalid2411"></a>Предполагаемая попытка повышения прав доступа к службе Netlogon (использование уязвимости CVE-2020-1472) (внешний идентификатор: 2411)
...
Если компьютер-источник является контроллером домена, неуспешное разрешение или разрешение с низкой достоверностью может препятствовать его дальнейшему выявлению службой Azure ATP.
