...
> * Предполагаемое использование Golden Ticket (аномалия билета) — предварительная версия (внешний код 2032)
...
> * Подозрительное изменение привилегированных групп (внешний код 2024)
...
1. Работает ли на компьютере-источнике ОС или приложение, которые имеют собственный механизм авторизации?  
    - Если компьютер-источник относится к этому типу, рекомендуется обновить операционную систему или исправить конфигурацию приложения. **Закройте** оповещение как действие **B-TP**. 
...
## <a name="suspected-golden-ticket-usage-ticket-anomaly-external-id-2032--preview"></a>Предполагаемое использование Golden Ticket (аномалия билета) (внешний код 2032) — предварительная версия 
...
3. Существует ли проблема синхронизации времени в сети (не все компьютеры синхронизированы)? 
    - Нажмите кнопку  **Загрузить сведения**, чтобы просмотреть Excel-файл отчета об оповещениях системы безопасности, просмотрите связанные сетевые операции и проверьте, есть ли разница между значениями StartTime и DomainControllerStartTime.
...
1. Уменьшите влияние исходного компьютера. 
    ...
    - Найдите пользователей, выполнивших вход в систему во время вредоносной активности, так как они также могут быть скомпрометированы. Сбросьте их пароли и включите многофакторную проверку подлинности.
    - При установке ATP в Защитнике Windows используйте команду **klist.exe purge**, чтобы удалить все билеты данного сеанса входа в систему и избежать использования билетов в дальнейшем.
3. Уменьшите влияние ресурсов, к которым осуществляется доступ с помощью этого билета.
4. Несколько раз меняйте пароль билета на получение билетов Kerberos (KRBTGT), как описано в записи блога о [скриптах для сброса пароля к учетной записи KRBTGT, доступных клиентам](https://cloudblogs.microsoft.com/microsoftsecure/2015/02/11/krbtgt-account-password-reset-scripts-now-available-for-customers/) с помощью [средства сброса пароля или ключей к учетной записи KRBTGT](https://gallery.technet.microsoft.com/Reset-the-krbtgt-account-581a9e51). 
    - Учтите, что такой сброс приведет к тому, что все билеты Kerberos в этом домене станут недействительными. Это означает, что работа **всех** служб будет нарушена и они не будут работать до обновления или, в некоторых случаях, до перезапуска службы. 
...
## <a name="suspicious-modification-of-sensitive-groups-external-id-2024"></a>Подозрительное изменение привилегированных групп (внешний код 2024)
...
Определение привилегированных групп в Azure ATP см. в разделе о работе с конфиденциальными учетными записями.
 
Обнаружение основано на аудите событий в контроллерах домена. Убедитесь, что контроллеры домена выполняют аудит необходимых событий.
