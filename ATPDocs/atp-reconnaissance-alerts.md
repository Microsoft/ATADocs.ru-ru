...
1. Если какая-либо попытка угадывания совпадает с существующими именами учетных записей, злоумышленник знает о существовании учетных записей в вашей среде и может попытаться использовать атаку методом подбора, чтобы получить доступ к домену с помощью обнаруженных имен пользователей. Исследуйте существующие учетные записи с помощью [руководства по анализу пользователей](investigate-a-user.md).
    > [!NOTE]
    > Проверьте свидетельство, чтобы узнать об используемом протоколе проверки подлинности. При использовании проверки подлинности NTLM включите аудит NTLM для События Windows 8004 на контроллере домена, чтобы определить сервер ресурсов, к которому пользователи пытались получить доступ.<br>
    > Событие Windows 8004 — это событие проверки подлинности NTLM, которое включает сведения об исходном компьютере, учетной записи пользователя и сервере, к которому предпринималась попытка доступа к исходной учетной записи пользователя. <br>
...
1. Уменьшите влияние [компьютера](investigate-a-computer.md)-источника. 
    ...
    1. Сбросьте их пароли и включите многофакторную проверку подлинности.
