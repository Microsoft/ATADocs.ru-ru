...
- Поля в порядке очередности:
    ...
    1. Метка времени фактического события (необходимо убедиться в том, что это значение не является меткой времени поступления на SIEM или отправки в ATP). Важно! Значение желательно представлять в миллисекундах.

    1. код события Windows.

    1. Имя поставщика событий Windows

    1. Имя журнала событий Windows

    1. Имя компьютера, получающего событие (в данном случае контроллера домена)

    1. Имя пользователя, подлинность которого проверяется

    1. Имя исходного узла

    1. Код результата NTLM
