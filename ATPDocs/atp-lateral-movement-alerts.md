...
> * Предполагаемые незаконные изменения при проверке подлинности NTLM (внешний ИД 2039) — предварительная версия
> * Потенциальная атака ретранслятора NTLM (учетная запись Exchange, внешний код 2037) — предварительная версия
...
При использовании [удаленного Credential Guard](https://docs.microsoft.com/windows/security/identity-protection/remote-credential-guard) для подключений на Windows Server 2016 и более поздних версиях к удаленному рабочему столу Windows 10 могут выводиться неопасные истинно положительные (**B-TP**) оповещения. Используя свидетельства из оповещения, проверьте, не выполнял ли пользователь подключение с исходного компьютера к удаленному рабочему столу конечного компьютера.
...
## <a name="suspected-ntlm-authentication-tampering-external-id-2039---preview"></a>Предполагаемые незаконные изменения при проверке подлинности NTLM (внешний ИД 2039) — предварительная версия
...
1.  Установлены ли на подверженных риску компьютерах, включая контроллеры домена, актуальные обновления и исправления для устранения уязвимости [CVE-2019-1040](https://portal.msrc.microsoft.com/security-guidance/advisory/CVE-2019-1040)? o   Если на компьютерах установлены актуальные обновления и исправления, ожидается, что проверка подлинности пройдена не будет. Если проверка подлинности не пройдена, **закройте** оповещение системы безопасности как неудачную попытку пройти проверку подлинности.
...
**Предотвращение** •   Проверьте, что на всех устройствах в среде установлены актуальные обновления и исправления для устранения уязвимости [CVE-2019-1040](https://portal.msrc.microsoft.com/security-guidance/advisory/CVE-2019-1040).

## <a name="suspected-ntlm-relay-attack-exchange-account-external-id-2037---preview"></a>Потенциальная атака ретранслятора NTLM (учетная запись Exchange, внешний код 2037) — предварительная версия

**Описание**

На Exchange Server можно настроить активируемую проверку подлинности NTLM с помощью учетной записи Exchange Server на удаленном HTTP-сервере, запущенном злоумышленником. Этот сервер ожидает соединения с Exchange Server, чтобы ретранслировать собственную конфиденциальную проверку подлинности на любой другой сервер или даже в Active Directory через LDAP, и извлекает сведения о проверке подлинности.
...
1. Уменьшите влияние компьютеров-источников
    1. Найдите средство, инициировавшее атаку, и удалите его.
    2. Найдите пользователей, выполнивших вход в систему во время вредоносной активности, так как они также могут быть скомпрометированы. Сбросьте их пароли и включите многофакторную проверку подлинности.
