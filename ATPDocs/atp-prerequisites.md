...
-   Учетная запись пользователя **локальной** службы Azure AD и пароль на доступ для чтения всех объектов в отслеживаемых доменах.
...
> [!NOTE]
> Требуется не менее 5 ГБ пространства; рекомендуется 10 ГБ. Сюда входит пространство, необходимое для двоичных файлов Azure ATP, журналов Azure ATP и журналов производительности.
...
-   **Адаптер управления** — будет использоваться для связи в корпоративной сети. При настройке этого адаптера следует настроить следующие параметры:
    ...
    -   статический IP-адрес, включая датчик по умолчанию;
...
Во время установки устанавливается платформа .NET Framework 4.7, что может приводить к перезагрузке контроллера домена.
