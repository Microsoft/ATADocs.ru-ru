...
- Необязательное требование. При развертывании автономного датчика следует пересылать события Windows 4776, 4732, 4733, 4728, 4729, 4756, 4757 и 7045 в Azure ATP, чтобы улучшить защиту от атак Pass-the-Hash, атак методом подбора, изменения конфиденциальных групп и создания вредоносных служб, а также оптимизировать выявление токенов-приманок (honeytoken). Датчик Azure ATP получает эти события автоматически. В автономном датчике Azure ATP эти события можно получать из системы SIEM или настроив пересылку событий Windows с контроллера домена. Собранные события предоставляют Azure ATP дополнительные данные, которые невозможно получить через сетевой трафик контроллера домена.
