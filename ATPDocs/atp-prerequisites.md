...
Обнаружения в Azure ATP основаны на определенных журналах событий Windows из контроллера домена, которые датчик может анализировать. Чтобы проводить аудит и включать в журнал событий Windows нужные события, контроллерам домена требуются точные параметры расширенной политики аудита. Дополнительные сведения см. в статье [Проверка расширенной политики аудита Azure ATP](atp-advanced-audit-policy.md).
