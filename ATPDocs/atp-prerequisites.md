...
[Портал Azure ATP](#azure-atp-workspace-management-portal-and-workspace-portal-requirements). В этом разделе описываются требования к браузеру для работы с порталом Azure ATP.
...
[Автономный датчик Azure ATP](#azure-atp-sensor-requirements). В этом разделе приводятся требования к программному и аппаратному обеспечению для автономных датчиков Azure ATP, а также указываются параметры, которые необходимо настроить на серверах этих датчиков.
...
-    Рекомендуемое: в контейнере удаленных объектов у пользователя должно быть разрешение только на чтение. Благодаря этому Azure ATP может обнаруживать удаление пользователей из Active Directory. Дополнительные сведения о настройке в контейнере удаленных объектов разрешений только на чтение см. в разделе **Изменение разрешений для контейнера удаленных объектов** статьи [View or Set Permissions on a Directory Object](https://technet.microsoft.com/library/cc816824%28v=ws.10%29.aspx) (Просмотр или установка разрешений для объекта каталога).

-   Дополнительно Учетная запись пользователя, не совершавшего операции в сети. Эта учетная запись будет настроена как пользователь Honeytoken Azure ATP. Дополнительные сведения см. в статье [Настройка исключений и пользователя Honeytoken](install-atp-step7.md).

-   Дополнительно При развертывании автономного датчика следует пересылать события Windows 4776, 4732, 4733, 4728, 4729, 4756, 4757 и 7045 в Azure ATP, чтобы улучшить защиту от атак Pass-the-Hash, атак методом подбора, изменения привилегированных групп и создания вредоносных служб, а также оптимизировать выявление honeytoken. Датчик Azure ATP получает эти события автоматически. В автономном датчике Azure ATP эти события можно получать из системы SIEM или настроив пересылку событий Windows с контроллера домена. Собранные события предоставляют Azure ATP дополнительные данные, которые невозможно получить через сетевой трафик контроллера домена.
...
|TLS для RDP-порта|TCP|3389|Все устройства в сети|Оба варианта|

### <a name="windows-event-logs"></a>Журналы событий Windows
Обнаружения в Azure ATP основаны на определенных журналах событий Windows из контроллера домена, которые датчик может анализировать. Чтобы проводить аудит и включать в журнал событий Windows нужные события, контроллерам домена требуются точные параметры расширенной политики аудита. См. дополнительные сведения о [проверке расширенной политики аудита](atp-advanced-audit-policy.md).
...
>   -   RDP (TCP-порт 3389), только первый пакет *приветствия клиента* в целях разрешения<br> Обратите внимание, что ни на одном порте не выполняется проверка подлинности.
...
|Syslog (необязательно)|TCP/UDP|514, в зависимости от конфигурации|Сервер SIEM|Входящие|
|RADIUS|UDP|1813|RADIUS|Входящие|
|TLS для RDP|TCP|3389|Все устройства в сети|Оба варианта|
