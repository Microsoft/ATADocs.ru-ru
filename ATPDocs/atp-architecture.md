...
> [!NOTE]
> - Если в вашей рабочей области в течение 60 дней не будет установлен датчик, рабочая область может быть удалена и вам потребуется создать ее заново.
...
Датчик Azure ATP считывает события локально. Для этого не нужно приобретать и обслуживать дополнительное оборудование или производить настройку. Датчик Azure ATP также поддерживает трассировку событий Windows (ETW), которая позволяет получить сведения журнала для нескольких обнаружений. Обнаруживаемые на основе ETW события включают подозрительный запрос на репликацию и подозрительное повышение роли контроллера домена, которые могут свидетельствовать об атаках DCShadow.
