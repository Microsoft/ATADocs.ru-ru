...
В этой статье содержатся ответы на часто задаваемые вопросы о решении Azure ATP.
...
ATP поддерживает механизм защиты Kerberos (также известный как безопасное туннелирование для гибкой аутентификации (FAST)). Однако в случае его активации ATP не сможет обнаруживать атаки Over-pass-the-hash.

## <a name="how-many-azure-atp-sensors-do-i-need"></a>Сколько требуется датчиков Azure ATP?

Для каждого контроллера домена в среде должен существовать датчик или автономный датчик ATP. Дополнительные сведения см. в разделе о [размерах датчиков Azure ATP](atp-capacity-planning.md#sizing). 
...
Azure ATP обнаруживает известные вредоносные атаки и методы нарушения безопасности, а также проблемы безопасности и риски.
Полный список обнаружений Azure ATP см. в статье [Какие угрозы обнаруживает Azure ATP?](suspicious-activity-guide.md)

## <a name="how-many-nics-does-the-azure-atp-standalone-sensor-require"></a>Сколько сетевых адаптеров требуется автономному датчику Azure ATP?
Автономному датчику Azure ATP необходимо как минимум два сетевых адаптера.<br>1. Сетевой адаптер для подключения к внутренней сети и облачной службе Azure ATP.<br>2. Сетевой адаптер используется для записи сетевого трафика контроллеров домена путем зеркального отображения порта.<br>* Это не относится к датчику Azure ATP, в котором изначально используются сетевые адаптеры, применяемые контроллером домена.
...
Azure ATP поддерживает двунаправленную интеграцию с системами SIEM.

1. В Azure ATP можно настроить отправку оповещений системного журнала на любой сервер системы SIEM, который поддерживает формат CEF, для уведомлений о состоянии работоспособности и при обнаружении подозрительной активности.
2. В Azure ATP можно настроить получение сообщений системного журнала о событиях Windows из [этих систем SIEM](configure-event-collection.md).
...
## <a name="can-azure-atp-monitor-domain-controllers-virtualized-on-your-iaas-solution"></a>Возможно ли с помощью Azure ATP отслеживать виртуализированные контроллеры домена в решении IaaS?
...
Сейчас это решение доступно в виде отдельного предложения. Оно не входит в состав Azure Active Directory или локальной версии Active Directory.
...
Вдобавок к анализу трафика Active Directory с использованием технологии тщательного анализа пакетов Azure ATP также выполняет сбор данных о соответствующих событиях из систем SIEM и создает профили сущностей на основе сведений из доменных служб Active Directory. Если вы используете датчик Azure ATP, он извлекает эти события автоматически. Для отправки этих событий в автономный датчик Azure ATP можно использовать пересылку событий Windows. Azure ATP также поддерживает получение данных учета RADIUS для журналов VPN от различных поставщиков (Microsoft, Cisco, F5 и Checkpoint).
...
При создании рабочей области Azure ATP можно выбрать возможность хранения данных в ней либо выбрать хранение данных в центрах обработки данных Microsoft Azure в США или в Европе. Настроенное место хранения данных изменить невозможно. Корпорация Майкрософт не будет переносить данные из указанного расположения. 

## <a name="is-my-data-isolated-from-other-customer-data"></a>Мои данные изолированы от данных других клиентов? 

Да, ваши данные изолированы посредством проверки подлинности для доступа и логического разделения на основе идентификатора клиента. Каждый клиент имеет доступ только к данным, собранным в его организации, а также к универсальным данным, предоставляемым корпорацией Майкрософт. 
