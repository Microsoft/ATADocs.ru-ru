...
|Оповещение|Описание:|Решение|Статус|
|----|----|----|----|
|Датчик Azure ATP получает больше событий, чем он может обработать.|Некоторые события Windows не анализируются, что может сказаться на возможности обнаружения подозрительных действий, выполняемых с контроллеров домена, которые отслеживаются этим датчиком Azure ATP.|Убедитесь в том, что только необходимые события пересылаются в датчик Azure ATP, или попытайтесь сделать так, чтобы некоторые события пересылались в другой датчик Azure ATP.|Средняя|
...
|Оповещение|Описание:|Решение|Статус|
|----|----|----|----|
|Датчик Azure ATP получает больше данных трассировки событий Windows (ETW), чем может обработать.|Некоторые события трассировки событий Windows (ETW) не анализируется. Это может затруднить обнаружение подозрительных действий, выполняемых на контроллерах домена, которые отслеживаются текущим датчиком Azure ATP.|Убедитесь в том, что только необходимые события пересылаются в датчик Azure ATP, или попытайтесь сделать так, чтобы некоторые события пересылались в другой датчик Azure ATP.|Средняя|
