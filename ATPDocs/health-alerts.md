...
|----|----|----|----|
|Датчик Azure ATP в настоящее время находится в автономном режиме из-за проблем с подключением ко всем настроенным контроллерам домена.|Это нарушает возможность Azure ATP использовать этот датчик для обнаружения подозрительных действий, связанных с отслеживаемыми им контроллерами.| Убедитесь в том, что контроллеры домена работают и что датчик Azure ATP может открывать подключения LDAP к ним. Также в области **Параметры** обязательно настройте учетную запись службы каталогов для каждого развернутого леса.|Средняя|
...
|Оповещение|Описание:|Решение|Статус|
|----|----|----|----|
|Датчик Azure ATP устарел.|Датчик Azure ATP работает под управлением версии, которая не может взаимодействовать с облачной инфраструктурой Azure ATP.|Обновите датчик вручную и выясните, почему он не обновляется автоматически. Если это не помогло, скачайте последний пакет установки датчика и переустановите датчик. Дополнительные сведения см. в разделе [Установка датчика Azure ATP](install-step4.md).|Средняя|
