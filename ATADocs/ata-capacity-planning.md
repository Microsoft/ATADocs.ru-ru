...
|----|----|----|----|-----|
|Шлюз ATA|Внешнее развертывание усложняет обнаружение ATA для злоумышленников|Высокие|Устанавливается вместе с контроллером домена (внешняя установка)|Поддерживает до 50 000 пакетов в секунду|
|Упрощенный шлюз ATA|Не требует выделенного сервера и настройки зеркального отражения портов|Низкие|Устанавливается на контроллере домена|Поддерживает до 10 000 пакетов в секунду|
...
> - При запуске шлюза в качестве виртуальной машины шлюзу необходимо, чтобы весь объем памяти был выделен для ВИРТУАЛЬНОЙ машины. Дополнительные сведения о запуске шлюза ATA в качестве виртуальной машины см. в разделе [требования к динамической памяти](https://docs.microsoft.com/advanced-threat-analytics/ata-prerequisites#dynamic-memory).
