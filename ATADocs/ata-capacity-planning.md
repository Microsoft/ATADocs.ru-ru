---
title: Планирование развертывания Advanced Threat Analytics
...
> - При запуске центра в качестве виртуальной машины центру требуется, чтобы вся память была выделена для ВИРТУАЛЬНОЙ машины. Дополнительные сведения о запуске центра ATA в качестве виртуальной машины см. в статье [требования к центру ATA](ata-prerequisites.md#dynamic-memory) .
...
|Тип шлюза|Преимущества|Затраты|Топология развертывания|Использование контроллера домена|
|----|----|----|----|-----|
...
&#42;&#42;Общее количество ядер без применения технологии Hyper-Threading, установленных на этом контроллере домена.  
Хотя технология Hyper-Threading является приемлемой для упрощенного шлюза ATA, при планировании производительности необходимо подсчитать фактические ядра и ядра без применения технологии Hyper-Threading.
...
> - При запуске шлюза в качестве виртуальной машины шлюзу необходимо, чтобы весь объем памяти был выделен для ВИРТУАЛЬНОЙ машины. Дополнительные сведения о запуске шлюза ATA в качестве виртуальной машины см. в разделе [требования к динамической памяти](ata-prerequisites.md#dynamic-memory).
...
> - Требуется не менее 5 ГБ пространства и рекомендуется 10 ГБ, включая пространство, необходимое для двоичных файлов ATA, [журналов ATA](troubleshooting-ata-using-logs.md)и [журналов производительности](troubleshooting-ata-using-perf-counters.md).
...
Для использования зеркального отображения портов может потребоваться развернуть несколько шлюзов ATA на шлюз данных или сайт филиала.
- **Производительность**  
