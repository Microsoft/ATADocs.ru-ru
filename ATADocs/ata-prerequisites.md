...
[Перед началом](#before-you-start)работы: В этом разделе перечислены сведения, которые необходимо собрать, а также учетные записи и сетевые объекты, которые должны быть созданы перед установкой ATA.

[Центр АТА](#ata-center-requirements). В этом разделе перечислены требования центра ATA к оборудованию и программному обеспечению, а также параметры, которые необходимо настроить на сервере центра ATA.
...
|Виртуальная машина запущена на|Описание|
|------------|-------------|
|Hyper-V|Убедитесь, что параметр **включить динамическая память** не включен для виртуальной машины.|
|Компании|Убедитесь, что настроенный объем памяти и зарезервированная память одинаковы, или выберите следующий параметр в параметре виртуальной машины — **резервировать всю гостевую память (все заблокированные)** .|
|Другой узел виртуализации|Сведения о том, как обеспечить полное выделение памяти виртуальной машине, см. в документации, предоставленной поставщиком. |
|
...
Для операционных систем Windows 2008R2 и 2012 шлюз не поддерживается в многопроцессорных [групповых](https://docs.microsoft.com/windows/win32/procthread/processor-groups) режимах. Дополнительные сведения о многопроцессорном режиме групп см. в разделе [Устранение неполадок](troubleshooting-ata-known-errors.md##multi-processor-group-mode). 
...
|------------|-------------|--------|-----------|-------------|
|**SSL** (ATA Communications)|TCP|443|Шлюз ATA|Входящий трафик|
|**HTTP** (необязательно)|TCP|80|Сеть организации|Входящий трафик|
|**HTTPS**|TCP|443|Сеть организации и шлюз ATA|Входящий трафик|
|**SMTP** (необязательно)|TCP|25|SMTP-сервер|Исходящее|
|**SMTPS** (необязательно)|TCP|465|SMTP-сервер|Исходящие|
|**Syslog** (необязательно)|TCP/ИБП/TLS (настраиваемые)|514 (по умолчанию)|Сервер Syslog|Исходящие|
...
|**Kerberos** (необязательно при присоединении к домену)|TCP и UDP|88|Контроллеры домена|Исходящие|
|**Служба времени Windows** (необязательно при присоединении к домену)|UDP|123|Контроллеры домена|Исходящие|
...
|SSL|TCP|443|Центр АТА|Исходящие|
|Syslog (необязательно)|UDP|514|Сервер SIEM|Входящий трафик|
...
Перед установкой упрощенного шлюза ATA на контроллере домена под управлением Windows Server 2012 R2 убедитесь, что установлено обновление [KB2919355](https://support.microsoft.com/kb/2919355/).
