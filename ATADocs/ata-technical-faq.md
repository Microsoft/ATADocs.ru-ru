...
1. Удаленное выполнение с использованием средства psexec.exe.
...
Следующий код можно поместить в файл, а затем выполнить его из командной строки в каталоге:  **\Program Files\Microsoft Advanced Threat Analytics\Center\MongoDB\bin** следующим образом:
...
На каждый полный день при среднем показателе 1000 пакетов/с требуется 0,3 ГБ свободного места в хранилище. Дополнительные сведения об определении размера центра ATA см. в статье [Планирование производительности ATA](ata-capacity-planning.md).
...
Если в виртуальном контроллере домена невозможно разместить упрощенный шлюз ATA, в нем можно реализовать физический или виртуальный шлюз ATA, как описано в статье [Настройка зеркального отображения портов](configure-port-mirroring.md).  

Проще всего настроить виртуальный шлюз ATA на каждом узле, где есть виртуальный контроллер домена. Если виртуальные контроллеры домена перемещаются между узлами, необходимо выполнить один из следующих шагов:
...
Для шлюза ATA необходимо как минимум два сетевых адаптера:<br>1. Сетевая карта для подключения к внутренней сети и центру ATA<br>2. Сетевая карта, используемая для записи сетевого трафика контроллера домена через зеркальное отображение портов.<br>* Это не относится к упрощенному шлюзу ATA, в котором изначально используются сетевые адаптеры, применяемые контроллером домена.
...
1. В ATA можно настроить получение сообщений системного журнала о событиях Windows из [этих систем SIEM](install-ata-step6.md).
...
Да, даже если решение ATA установили после нарушения, оно все равно обнаружит подозрительные действия злоумышленников. ATA не только просматривает поведение пользователя, но и для других пользователей в схеме безопасности Организации. Во время первоначального анализа, если поведение злоумышленника является ненормальным, оно определяется как "выброс", а ATA сохраняет отчеты о нормальном поведении. Кроме того, если злоумышленник пытается украсть учетные данные пользователей, например при атаке Pass-the-Ticket, или удаленно выполнить действие на одном из контроллеров домена, ATA также определит эти действия как подозрительные.
