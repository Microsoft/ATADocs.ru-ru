...
Advanced Threat Analytics (ATA) — это локальная платформа, которая помогает защитить предприятие от нескольких типов сложных целевых кибератакам и угроз для участников. Чтобы получить дополнительные сведения об ATA, воспользуйтесь следующими ресурсами.
...
[Средство изменения размера ATA](ata-capacity-planning.md) . средство изменения размера автоматизирует сбор данных о количестве потребностей в ATA. Он автоматически обеспечивает поддержку и рекомендует ресурсы как для центра ATA, так и для упрощенных шлюзов ATA.
...
[Руководство по подтверждению концепции ATA](/samples/browse/?redirectedfrom=TechNet-Gallery) — содержит все шаги, необходимые для развертывания ATA в качестве подтверждения концепции.
...
[Подозрительное действие ATA сборник тренировочных заданий](/samples/browse/?redirectedfrom=TechNet-Gallery) . в этой статье рассматриваются методы атаки путем кражи учетных данных с помощью общедоступных средств исследования в Интернете. Вы узнаете, каким образом ATA помогает вам увидеть такие угрозы в каждой точке атаки.
