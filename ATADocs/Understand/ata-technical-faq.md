...
В командной строке каталога **\Program Files\Microsoft Advanced Threat Analytics\Center\MongoDB\bin** можно выполнить следующие действие:

        mongo ATA --eval "printjson(db.getCollectionNames())" | find /C "NtlmEvents"`
## Поддерживает ли ATA зашифрованный трафик?
Зашифрованный трафик не анализируется (например, LDAPS, IPSEC ESP).
...
При определении числа шлюзов ATA для контролеров домена, где нельзя разместить упрощенный шлюз ATA, учитывайте следующее:

 - Общий объем трафика, создаваемый контроллерами домена, и сетевую архитектуру (для настройки зеркального отображения портов). Дополнительные сведения о том, как определить, какой объем трафика создается контроллерами домена, см. в разделе [Оценка трафика контроллера домена](/advanced-threat-analytics/plan-design/ata-capacity-planning#Domain-controller-traffic-estimation).
 - Количество шлюзов ATA, которое необходимо для поддержки контроллеров домена, (для коммутатора, центра обработки данных и региона) зависит от ограничений зеркального отображения портов. Каждая среда имеет свои собственные требования. 
...
-   Данные о конфигурации ATA, которые хранятся в базе данных и автоматически архивируются каждый час. 
...
Полный список атак и угроз, которые может обнаружить ATA, см. в статье [Что представляет собой решение Microsoft Advanced Threat Analytics?](what-is-ata.md).
...
Общедоступная версия Microsoft Advanced Threat Analytics поддерживает несколько доменов в пределах одного периметра леса. По сути лес — это периметр безопасности, поэтому поддержка нескольких доменов позволит нашим клиентам полностью защитить свои среды с помощью решения ATA.
